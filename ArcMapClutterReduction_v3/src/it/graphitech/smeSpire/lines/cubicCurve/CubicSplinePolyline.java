		...
		try {
			...
			if (isFirstNodeInsideLense() && !isLastNodeInsideLense()) {
				...
				if (!isRenderUndesiredArcs) {
					...
					Color col = getColorFromNodePosition(getFrom());
...
gl.glLineWidth((float) SharedVariables.lineWidth);
					...
					/*
					 * short lineType = (short) 0xF0F0; gl.glLineStipple(4,
					 * lineType); gl.glEnable(GL2.GL_LINE_STIPPLE);
					 * drawLines(gl, geometryType,
					 * myRendLayer.getUpdBufferElement(lineIndex),
					 * bufferUpdPointSize, 0);
					 * gl.glDisable(GL2.GL_LINE_STIPPLE);
					 * 
					 * setColor(gl, colorsFormodifiedArc[1]); short lineType2 =
					 * 0xF0F; gl.glLineStipple(4, lineType2);
					 * gl.glEnable(GL2.GL_LINE_STIPPLE); drawLines(gl,
					 * geometryType, myRendLayer.getUpdBufferElement(lineIndex),
					 * bufferUpdPointSize, 0);
					 * gl.glDisable(GL2.GL_LINE_STIPPLE);
					 */
...
//contex arc -> draw arc
				...
				drawLines(gl, myRendLayer.getBufferAllPointsIndex(),
						numVerticesPerLine, lineIndex * numVerticesPerLine);
				// drawLines(gl,geometryType,myRendLayer.getBufferElement(lineIndex),numVerticesPerLine,0);
