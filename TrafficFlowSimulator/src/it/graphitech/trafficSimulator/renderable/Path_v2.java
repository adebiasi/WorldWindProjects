...
/**
 * 
 * This is update of the class gov.nasa.worldwind.render.Path
 * 
 * Displays a line or curve between positions. The path is drawn between input
 * positions to achieve a specified path type, e.g., {@link AVKey#GREAT_CIRCLE}.
 * It can also conform to the underlying terrain. A curtain may be formed by
 * extruding the path to the ground.
 */
public class Path_v2 extends AbstractShape {
	...
	/**
	 * The offset applied to a terrain following Path's depth values to to
	 * ensure it shows over the terrain: 0.99. Values less than 1.0 pull the
	 * path in front of the terrain, values greater than 1.0 push the path
	 * behind the terrain.
	 */
	...
	/**
	 * Maintains globe-dependent computed data such as Cartesian vertices and
	 * extents. One entry exists for each distinct globe that this shape
	 * encounters in calls to {@link AbstractShape#render(DrawContext)}. See
	 * {@link AbstractShape}.
	 */
	protected static class PathData extends AbstractShapeData {
		...
		/**
		 * The model coordinate vertices to render, all relative to this shape
		 * data's reference center. If the path is extruded, the base vertices
		 * are interleaved: Vcap, Vbase, Vcap, Vbase, ...
		 */
		protected FloatBuffer renderedPath;
		/**
		 * Indices in the <code>renderedPath</code> identifying the vertices of
		 * the originally specified boundary positions. This is used to draw
		 * vertical lines at those positions when the path is extruded.
		 */
		protected IntBuffer polePositions; // identifies original positions in
											// rendered path
		/**
		 * Indicates whether the rendered path has extrusion points in addition
		 * to path points.
		 */
		protected boolean hasExtrusionPoints; // true when the rendered path
												// contains extrusion points

		public PathData(DrawContext dc, Path_v2 shape) {
	...
	protected Iterable<? extends Position> positions; // the positions as
														// provided by the
														// application
	protected int numPositions; // the number of positions in the positions
								// field.

	protected String pathType = DEFAULT_PATH_TYPE;
	protected boolean followTerrain; // true if altitude mode indicates terrain
										// following
	...
	protected double incrementElevation = 0;
	 ...
	 * 
	 * @param positions
	 *            the path positions. This reference is retained by this shape;
	 *            the positions are not copied. If any positions in the set
	 *            change, {@link #setPositions(Iterable)} must be called to
	 *            inform this shape of the change.
	 * 
	 * @throws IllegalArgumentException
	 *             if positions is null.
	 */
	public Path_v2(Iterable<? extends Position> positions) {
	 ...
	 *             if positions is null.
	 */
	public Path_v2(Position.PositionList positions) {
		if (positions == null) {
	 ...
	 */
	public Path_v2(Position posA, Position posB) {
	 ...
	 * 
	 * @return this path's positions. Will be null if no positions have been
	 *         specified.
	 */
	...
	/**
	 * Specifies this path's positions, which replace this path's current
	 * positions, if any.
	 * <p/>
	 ...
	 * 
	 * @param extrude
	 *            true to extrude this path, otherwise false. The default value
	 *            is false.
	 */
	 ...
	 * 
	 * @param followTerrain
	 *            true if terrain following, otherwise false. The default value
	 *            is false.
	 */
	...
	/**
	 * Indicates the number of segments used between specified positions to
	 * achieve this path's path type. Higher values cause the path to conform
	 * more closely to the path type but decrease performance.
	 * <p/>
	 * Note: The sub-segments number is ignored when the path follows terrain or
	 * when the path type is {@link AVKey#LINEAR}.
	 * 
	...
	/**
	 * Specifies the number of segments used between specified positions to
	 * achieve this path's path type. Higher values cause the path to conform
	 * more closely to the path type but decrease performance.
	 * <p/>
	 * Note: The sub-segments number is ignored when the path follows terrain or
	...
	/**
	 * Indicates the terrain conformance target when this path follows the
	 * terrain. The value indicates the maximum number of pixels between which
	 * intermediate positions of a path segment -- the path portion between two
	 * specified positions -- are computed.
	 * 
	...
	/**
	 * Specifies how accurately this path must adhere to the terrain when the
	 * path is terrain following. The value specifies the maximum number of
	 * pixels between tessellation points. Lower values increase accuracy but
	...
	/**
	 * Specifies this path's path type. Recognized values are
	 * {@link AVKey#GREAT_CIRCLE}, {@link AVKey#RHUMB_LINE} and
	 * {@link AVKey#LINEAR}.
	 * 
	 * @param pathType
	 *            the current path type. The default value is
	 *            {@link AVKey#LINEAR}.
	 */
	...
	/**
	 * Indicates whether to draw at each specified path position when this path
	 * is extruded.
	 * 
	...
	/**
	 * Specifies whether to draw vertical lines at each specified path position
	 * when this path is extruded.
	 * 
	 * @param drawVerticals
	 *            true to draw the lines, otherwise false. The default value is
	 *            true.
	 */
	...
	protected boolean mustDrawInterior() {
		return super.mustDrawInterior()
				&& this.getCurrentPathData().hasExtrusionPoints;
	}
	...
	protected boolean mustRegenerateGeometry(DrawContext dc) {
		if (this.getCurrentPathData() == null
				|| this.getCurrentPathData().renderedPath == null)
			return true;

		if (this.getCurrentPathData().tessellatedPositions == null)
			return true;

		if (dc.getVerticalExaggeration() != this.getCurrentPathData()
		...
		if (this.getAltitudeMode() == WorldWind.ABSOLUTE
				...
				&& this.getCurrentPathData().getGlobeStateKey()
						.equals(dc.getGlobe().getGlobeStateKey(dc)))
			return false;
	...
	protected boolean shouldUseVBOs(DrawContext dc) {
		return this.getCurrentPathData().tessellatedPositions.size() > VBO_THRESHOLD
				&& super.shouldUseVBOs(dc);
	}

	/**
	 * Indicates whether this Path's defining positions and the positions in
	 * between are located on the underlying terrain. This returns
	 * <code>true</code> if this Path's altitude mode is
	 * <code>WorldWind.CLAMP_TO_GROUND</code> and the follow-terrain property is
	 * <code>true</code>. Otherwise this returns <code>false</code>.
	 * 
	 * @return <code>true</code> if this Path's positions and the positions in
	 *         between are located on the underlying terrain, and
	 *         <code>false</code> otherwise.
	 */
	protected boolean isSurfacePath() {
		return this.getAltitudeMode() == WorldWind.CLAMP_TO_GROUND
				&& this.isFollowTerrain();
	}

	@Override
	protected void determineActiveAttributes() {
		// When the interior is drawn the vertex buffer has a different layout,
		// so it may need to be rebuilt.
		boolean isDrawInterior = this.activeAttributes.isDrawInterior();

		super.determineActiveAttributes();

		if (this.activeAttributes != null
				&& this.activeAttributes.isDrawInterior() != isDrawInterior)
	...
	protected boolean doMakeOrderedRenderable(DrawContext dc) {
		...
		// Recompute tessellated positions because the geometry or view may have
		// changed.
		this.makeTessellatedPositions(dc, pathData);
		if (pathData.tessellatedPositions == null
				|| pathData.tessellatedPositions.size() < 2)
			return false;

		// Create the rendered Cartesian points.
		int previousSize = pathData.renderedPath != null ? pathData.renderedPath
				.limit() : 0;
		...
		if (pathData.renderedPath.limit() > previousSize
				&& this.shouldUseVBOs(dc))
	 ...
	 * {@inheritDoc}
	 * <p/>
	 * Overridden to place this Path behind other ordered renderables when this
	 * Path is entirely located on the underlying terrain. In this case this
	 * Path must be drawn first to ensure that other ordered renderables are
	 * correctly drawn on top of it and are not affected by this Path's depth
	 * offset. If two paths are both located on the terrain, they are drawn with
	 * respect to their layer ordering.
	 */
	@Override
	protected void addOrderedRenderable(DrawContext dc) {
		if (this.isSurfacePath()) {
			dc.addOrderedRenderable(this, true); // Specify that this Path is
													// behind before other
													// renderables.
		} else {
	...
	protected boolean isOrderedRenderableValid(DrawContext dc) {
		return this.getCurrentPathData().renderedPath != null
				&& this.getCurrentPathData().renderedPath.limit() >= 6;
	}

	/**
	 * {@inheritDoc}
	 * <p/>
	 * If this Path is entirely located on the terrain, this applies an offset
	 * to the Path's depth values to to ensure it shows over the terrain. This
	 * does not apply a depth offset in any other case to avoid incorrectly
	 * drawing the path over objects it should be behind, including the terrain.
	 * In addition to applying a depth offset, this disables writing to the
	 * depth buffer to avoid causing subsequently drawn ordered renderables to
	 * incorrectly fail the depth test. Since this Path is located on the
	 * terrain, the terrain already provides the necessary depth values and we
	 * can be certain that other ordered renderables should appear on top of it.
	 */
	@Override
	protected void doDrawOutline(DrawContext dc) {
		...
		try {
			if (this.isSurfacePath()) {
				// Pull the line forward just a bit to ensure it shows over the
				// terrain.
	...
	protected void doDrawOutlineVBO(DrawContext dc, int[] vboIds,
			PathData pathData) {
		GL gl = dc.getGL();

		gl.glBindBuffer(GL.GL_ARRAY_BUFFER, vboIds[0]);
		gl.glVertexPointer(3, GL.GL_FLOAT,
				pathData.hasExtrusionPoints ? 24 : 0, 0);
		gl.glDrawArrays(GL.GL_LINE_STRIP, 0, pathData.renderedPath.limit()
				/ (pathData.hasExtrusionPoints ? 6 : 3));
	...
	protected void doDrawOutlineVA(DrawContext dc, PathData pathData) {
		...
		gl.glVertexPointer(3, GL.GL_FLOAT,
				pathData.hasExtrusionPoints ? 24 : 0,
	...
	protected void drawVerticalOutlineVBO(DrawContext dc, int[] vboIds,
			PathData pathData) {
		...
		gl.glDrawElements(GL.GL_LINES, polePositions.limit(),
	...
	protected void drawVerticalOutlineVA(DrawContext dc, PathData pathData) {
		...
		dc.getGL().glVertexPointer(3, GL.GL_FLOAT, 0,
				pathData.renderedPath.rewind());
		dc.getGL().glDrawElements(GL.GL_LINES, polePositions.limit(),
				GL.GL_UNSIGNED_INT, polePositions.rewind());
	}
	...
	protected void doDrawInteriorVA(DrawContext dc, PathData pathData) {
		dc.getGL().glVertexPointer(3, GL.GL_FLOAT, 0,
				pathData.renderedPath.rewind());
		dc.getGL().glDrawArrays(GL.GL_TRIANGLE_STRIP, 0,
				pathData.renderedPath.limit() / 3);
	}

	/**
	 * Computes the shape's model-coordinate path from a list of positions.
	 * Applies the path's terrain-conformance settings. Adds extrusion points --
	 * those on the ground -- when the path is extruded.
	 * 
	 * @param dc
	 *            the current draw context.
	 * @param positions
	 ...
	 *            the current globe-specific path data.
	 */
	protected void computePath(DrawContext dc, List<Position> positions,
			PathData pathData) {
		...
		if (this.getAltitudeMode() == WorldWind.CLAMP_TO_GROUND) {
			path = this.computePointsRelativeToTerrain(dc, positions, 0d, path,
					pathData);
		} else if (this.getAltitudeMode() == WorldWind.RELATIVE_TO_GROUND) {
			path = this.computePointsRelativeToTerrain(dc, positions, null,
					path, pathData);
		} else {
			path = this.computeAbsolutePoints(dc, positions, path, pathData);
		}

		path.flip(); // since the path is reused the limit might not be the same
						// as the previous usage

		pathData.renderedPath = path;
	}

	/**
	 * Computes a terrain-conforming, model-coordinate path from a list of
	 * positions, using either a specified altitude or the altitudes in the
	 * specified positions. Adds extrusion points -- those on the ground -- when
	 * the path is extruded and the specified single altitude is not 0.
	 * 
	 * @param dc
	 *            the current draw context.
	 * @param positions
	 *            the positions to create a path for.
	 * @param altitude
	 *            if non-null, the height above the terrain to use for all
	 *            positions. If null, each position's altitude is used as the
	 *            height above the terrain.
	 * @param path
	 *            a buffer in which to store the computed points. May be null.
	 *            The buffer is not used if it is null or tool small for the
	 *            required number of points. A new buffer is created in that
	 *            case and returned by this method. This method modifies the
	 ...
	 */
	protected FloatBuffer computePointsRelativeToTerrain(DrawContext dc,
			List<Position> positions, Double altitude, FloatBuffer path,
			PathData pathData) {
		boolean extrudeIt = this.isExtrude()
		...
		for (Position pos : positions) {
			...
			Vec4 pt = dc.computeTerrainPoint(pos.getLatitude(),
					pos.getLongitude(), height + incrementElevation);
	...
	/**
	 * Computes a model-coordinate path from a list of positions, using the
	 * altitudes in the specified positions. Adds extrusion points -- those on
	 * the ground -- when the path is extruded and the specified single altitude
	 * is not 0.
	 * 
	 * @param dc
	 *            the current draw context.
	 ...
	 * @return the buffer in which to place the computed points.
	 */
	protected FloatBuffer computeAbsolutePoints(DrawContext dc,
			List<Position> positions, FloatBuffer path, PathData pathData) {

		int numPoints = this.isExtrude() ? 2 * positions.size() : positions
				.size();
		...
		if (dc.getVerticalExaggeration() != 1) {
			...
			for (Position pos : positions) {
				Vec4 pt = globe.computePointFromPosition(pos.getLatitude(),
						pos.getLongitude(), ve
								* (pos.getAltitude() + incrementElevation));
				path.put((float) (pt.x - referencePoint.x));
				path.put((float) (pt.y - referencePoint.y));
				path.put((float) (pt.z - referencePoint.z));
	...
	public double getIncrementElevation() {
		return incrementElevation;
	}

	public void setIncrementElevation(double incrementElevation) {
		this.incrementElevation = incrementElevation;
	}

	/**
	 * Computes a point on a path and adds it to the renderable geometry. Used
	 * to generate extrusion vertices.
	 * 
	 * @param dc
	 *            the current draw context.
	 * @param position
	 *            the path position.
	 * @param path
	 *            the path to append to. Assumes that the path has adequate
	 *            capacity.
	 * @param pathData
	 *            the current globe-specific path data.
	 */
	protected void appendTerrainPoint(DrawContext dc, Position position,
			FloatBuffer path, PathData pathData) {
		Vec4 referencePoint = pathData.getReferencePoint();
		Vec4 pt = dc.computeTerrainPoint(position.getLatitude(),
				position.getLongitude(), 0d);
		path.put((float) (pt.x - referencePoint.x));
		path.put((float) (pt.y - referencePoint.y));
		path.put((float) (pt.z - referencePoint.z));
	...
	/**
	 * Generates positions defining this path with path type and
	 * terrain-conforming properties applied. Builds the path's
	...
	protected void makeTessellatedPositions(DrawContext dc, PathData pathData) {
		...
		if (pathData.tessellatedPositions == null) {
			int size = (this.numSubsegments * (this.numPositions - 1) + 1)
					* (this.isExtrude() ? 2 : 1);
		...
		if (pathData.polePositions == null
				|| pathData.polePositions.capacity() < this.numPositions * 2)
			pathData.polePositions = BufferUtil
					.newIntBuffer(this.numPositions * 2);
		else
			pathData.polePositions.clear();

		Iterator<? extends Position> iter = this.positions.iterator();
		Position posA = iter.next();
		this.addTessellatedPosition(posA, true, pathData); // add the first
															// position of the
															// path
		...
		for (int i = 1; i <= this.numPositions; i++) {
			...
			// If the segment is very small or not visible, don't tessellate it,
			// just add the segment's end position.
			if (this.isSmall(dc, ptA, ptB, 8)
					|| !this.isSegmentVisible(dc, posA, posB, ptA, ptB))
	...
	/**
	 * Adds a position to this path's <code>tessellated</code> list and
	 * optionally its <code>polePositions</code> list.
	 * 
	 * @param pos
	 *            the position to add.
	 * @param polePosition
	 *            if true, add the positions index to the
	 *            <code>polePositions</code>.
	 * @param pathData
	 *            the current globe-specific path data.
	 */
	protected void addTessellatedPosition(Position pos, boolean polePosition,
			PathData pathData) {
		if (polePosition) {
		...
		pathData.tessellatedPositions.add(pos); // be sure to do the add after
												// the pole position is set
	}
	 ...
	 * @param ptA
	 *            the model-coordinate point corresponding to the segment's
	 *            first position.
	 * @param ptB
	 *            the model-coordinate point corresponding to the segment's
	 *            second position.
	 * 
	 * @return true if the segment is visible relative to the current view
	 *         frustum, otherwise false.
	 */
	...
	/**
	 * Creates the interior segment positions to adhere to the current path type
	 * and terrain-following settings.
	 * 
	 * @param dc
	 *            the current draw context.
	 * @param posA
	...
	protected void makeSegment(DrawContext dc, Position posA, Position posB,
			Vec4 ptA, Vec4 ptB, PathData pathData) {
		// This method does not add the first position of the segment to the
		// position list. It adds only the
		// subsequent positions, including the segment's last position.

		double arcLength = this.getPathType() == AVKey.LINEAR ? ptA
				.distanceTo3(ptB) : this.computeSegmentLength(dc, posA, posB);
		if (arcLength <= 0
				|| (this.getPathType() == AVKey.LINEAR && !this
						.isFollowTerrain())) {
			if (!ptA.equals(ptB))
				this.addTessellatedPosition(posB, true, pathData);
			return;
		...
		for (double s = 0, p = 0; s < 1;) {
			if (this.isFollowTerrain())
				p += this.terrainConformance
						* dc.getView().computePixelSizeAtDistance(
			...
			if (s >= 1) {
				pos = posB;
			} else if (this.pathType == AVKey.RHUMB_LINE
					|| this.pathType == AVKey.LINEAR) // or LOXODROME
			{
				...
				LatLon latLon = LatLon.rhumbEndPosition(posA, segmentAzimuth,
						distance);
				pos = new Position(latLon, (1 - s) * posA.getElevation() + s
						* posB.getElevation());
			} else // GREAT_CIRCLE
			{
				if (segmentAzimuth == null) {
				...
				Angle distance = Angle.fromRadians(s * segmentDistance.radians);
				LatLon latLon = LatLon.greatCircleEndPosition(posA,
						segmentAzimuth, distance);
				pos = new Position(latLon, (1 - s) * posA.getElevation() + s
						* posB.getElevation());
			}
	...
	/**
	 * Computes the approximate model-coordinate, great-circle length between
	 * two positions.
	 * 
	 * @param dc
	 *            the current draw context.
	 ...
	 */
	protected double computeSegmentLength(DrawContext dc, Position posA,
			Position posB) {
	...
	/**
	 * Computes the path's bounding box from the current rendering path. Assumes
	 * the rendering path is up-to-date.
	 * 
	 * @param current
	...
	protected Extent computeExtent(PathData current) {
		...
		Box box = Box.computeBoundingBox(new BufferWrapper.FloatBufferWrapper(
				current.renderedPath));

		// The path points are relative to the reference center, so translate
		// the extent to the reference center.
	...
	public Extent getExtent(Globe globe, double verticalExaggeration) {
		...
		// Use the tessellated positions if they exist because they best
		// represent the actual shape.
		Iterable<? extends Position> posits = current.tessellatedPositions != null ? current.tessellatedPositions
				: this.getPositions();
		if (posits == null)
			return null;

		return super.computeExtentFromPositions(globe, verticalExaggeration,
				posits);
	}

	/**
	 * Computes the path's reference position. The position returned is the
	 * center-most ordinal position in the path's specified positions.
	 * 
	 * @return the computed reference position.
	 */
	public Position getReferencePosition() {
		return this.numPositions < 1 ? null : this.positions.iterator().next(); // use
																				// the
																				// first
																				// position
	}
	...
	protected void fillVBO(DrawContext dc) {
		PathData pathData = this.getCurrentPathData();
		int numIds = pathData.hasExtrusionPoints && this.isDrawVerticals() ? 2
				: 1;

		int[] vboIds = (int[]) dc.getGpuResourceCache().get(
		...
		int iSize = pathData.hasExtrusionPoints && this.isDrawVerticals() ? this.numPositions * 2 * 4
				: 0;

		if (vboIds == null) {
			vboIds = new int[numIds];
			gl.glGenBuffers(vboIds.length, vboIds, 0);
			dc.getGpuResourceCache().put(pathData.getVboCacheKey(), vboIds,
					GpuResourceCache.VBO_BUFFERS, vSize + iSize);
		}

		try {
			FloatBuffer vb = pathData.renderedPath;
			gl.glBindBuffer(GL.GL_ARRAY_BUFFER, vboIds[0]);
			gl.glBufferData(GL.GL_ARRAY_BUFFER, vb.limit() * 4, vb.rewind(),
					GL.GL_STATIC_DRAW);

			if (pathData.hasExtrusionPoints && this.isDrawVerticals()) {
				IntBuffer ib = pathData.polePositions;
				gl.glBindBuffer(GL.GL_ELEMENT_ARRAY_BUFFER, vboIds[1]);
				gl.glBufferData(GL.GL_ELEMENT_ARRAY_BUFFER, ib.limit() * 4,
						ib.rewind(), GL.GL_STATIC_DRAW);
			}
		} finally {
			gl.glBindBuffer(GL.GL_ARRAY_BUFFER, 0);
			gl.glBindBuffer(GL.GL_ELEMENT_ARRAY_BUFFER, 0);
	...
	public void move(Position delta) {
		...
		// The reference position is null if this Path has no positions. In this
		// case moving the Path by a
		// relative delta is meaningless because the Path has no geographic
	...
	public void moveTo(Position position) {
		...
		// The reference position is null if this Path has no positions. In this
		// case moving the Path to a new
		// reference position is meaningless because the Path has no geographic
		// location. Therefore we fail softly
		// by exiting and doing nothing.
		if (oldPosition == null)
			return;

		List<Position> newPositions = Position.computeShiftedPositions(
				oldPosition, position, this.positions);
	...
	protected boolean isSmall(DrawContext dc, Vec4 ptA, Vec4 ptB, int numPixels) {
		return ptA.distanceTo3(ptB) <= numPixels
				* dc.getView().computePixelSizeAtDistance(
	...
	protected void doExportAsKML(XMLStreamWriter xmlWriter) throws IOException,
		...
		final String altitudeMode = KMLExportUtil
				.kmlAltitudeMode(getAltitudeMode());
		...
		for (Position position : this.positions) {
			xmlWriter.writeCharacters(String.format("%f,%f,%f ", position
					.getLongitude().getDegrees(), position.getLatitude()
					.getDegrees(), position.getElevation()));
		}
		xmlWriter.writeEndElement();
