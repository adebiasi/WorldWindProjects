...
//Position newPos = Position.fromDegrees(pos.getLatitude().degrees, pos.getLongitude().degrees);
        // Scale and opacity depending on distance from eye
        double[] scaleAndOpacity = computeDistanceScaleAndOpacity(dc, point, size);

        this.setDepthFunc(dc, screenPoint);
        this.drawNotTopLevelAnnotation(dc, (int) screenPoint.x, (int) screenPoint.y, size.width, size.height,
            scaleAndOpacity[0], scaleAndOpacity[1], pos);
    }

    
    
    protected void drawNotTopLevelAnnotation(DrawContext dc, int x, int y, int width, int height, double scale,
            double opacity, Position pickPosition)
        {
            ...
            try
            {
                ...
                //System.out.println("pick position: "+pickPosition);
    ...
    protected void applyScreenTransform(DrawContext dc, int x, int y, int width, int height, double scale)
    {
        ...
        gl.glScaled(finalScale, finalScale, 0);
    ...
    protected void setDepthFunc(DrawContext dc, Vec4 screenPoint)
    {
       ...
       boolean isVisible =  SharedVariables.isPositionVisible(dc, SharedVariables.computeWorldCoordinatesFromScreenPoint(screenPoint));
