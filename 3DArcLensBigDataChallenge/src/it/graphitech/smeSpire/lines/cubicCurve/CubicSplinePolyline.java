...
String time;
	...
	int numberCalls;
	int totNumberCalls;
	double minNumberCalls;
	double maxNumberCalls;
	double minTotNumberCalls;
	double maxTotNumberCalls;
	
	int maxNumberCallsPerState;
	int minNumberCallsPerState;
	
	boolean isNumberCallsOutliner;
	boolean isAvgTimeOutliner;
	
	int totNumberCallsOutliner;
	int totAvgTimeOutliner;
	Color colorAvgTime;
	Color colorNumberCalls;
	//Color colorOutlineAvgTime;
	Color colorOutlineNumberCalls;
	Color colorPercentNumberCalls;
	...
	protected Color colorsForModifiedArc = Color.WHITE;
		...
		try {
			...
			if (isFirstNodeInsideLense() && !isLastNodeInsideLense()) {
				...
				if (!isRenderUndesiredArcs) {
					...
					Color col = colorForUnmodifiedArc;
...
//System.out.println("destination inside lens -> filter arc");
			...
			if (isLastNodeInsideLense() && isFirstNodeInsideLense()) {
				...
				if (!isRenderUndesiredArcs) {
					...
					/*
					//	System.out.println("origin and destination inside lens -> draw arc");
					gl.glDisable(GL2.GL_DEPTH_TEST);
					SharedVariables.counterOfLinesInsideLens++;
					//drawNode(gl, getTo(), lastNodeWorldPosition);
					//drawNode(gl, getFrom(), firstNodeWorldPosition);
...
//undesired arc -> deform arc
			if (isAffectedByLense() & SharedVariables.lensIsActive) {
				...
				if (isRenderUndesiredArcs) {
//System.out.println("undesired arc -> deform arc");

					
					deformArc(gl);
					/*
					gl.glLineWidth((float) SharedVariables.lineWidth);
					setToModifiedLinesColor(gl, colorsForModifiedArc);
			...
			*/
			...
			else if (isAffectedByLense() & !SharedVariables.lensIsActive) {
...
/////////////////////////
/////////////////////////Color col = getColorFromNodePosition(getTo());
/////////////////////////
				Color col = colorForUnmodifiedArc;
				setColorToLinesPartiallyInsideTheLens(gl, col);
				myRendLayer.useShader(gl);
...
//System.out.println("undesired arc -> deform arc");
	...
	private void filterArc(GL2 gl){
		...
		setColorToLinesPartiallyInsideTheLens(gl, colorForUnmodifiedArc);
		setWidthToContextLines(gl);
		drawLines(gl, myRendLayer.getBufferAllPointsIndex(),
		numVerticesPerLine, lineIndex * numVerticesPerLine);
	...
	private void unfilterArc(GL2 gl){
		...
		setColorToLinesCompletelyInsideLens(gl, colorForUnmodifiedArc);
		setWidthToContextLines(gl);
		drawLines(gl, myRendLayer.getBufferAllPointsIndex(),
				numVerticesPerLine, lineIndex * numVerticesPerLine);
...
/*
	private void drawNode(GL2 gl, String nodePosition, Vec4 pos) {

		Color col = getColorFromNodePosition(nodePosition);
		setColorToContextLines(gl, col);
		drawNode(gl, pos, col);

	}
*/
	private void drawNode(GL2 gl, Vec4 pos, Color col) {
		GLUT glut = new GLUT();
		...
		try {
			...
			// drawQuad(gl);
			glut.glutSolidSphere(SharedVariables.nodeDiameter, 4, 4);
		} finally {
			ogsh.pop(gl);
	...
	double percentCalls = ((double)(totNumberCalls-minTotNumberCalls))/((double)(maxTotNumberCalls-minTotNumberCalls));
	//System.out.println("percentCalls: "+percentCalls);
	double updatedWidth = percentCalls*10;
	//gl.glLineWidth((float) SharedVariables.lineWidth);
	gl.glLineWidth((float) updatedWidth);
	...
	//	  color.getBlue(), (byte) color.getAlpha());
	...
	public CubicSplinePolyline(RenderableControlPoints renderableControlPoints) {
		...
		this.updRenderableControlPoints
				.setControlPoint((ArrayList<Vec4>) renderableControlPoints.points
						.clone());
		start = renderableControlPoints.points.get(0);
		end = renderableControlPoints.points.get(renderableControlPoints.points.size()-1);
	}
...
*/
		...
		if (!isFirstNodeInsideLense() & !isLastNodeInsideLense()) {
			...
			if (isAffectedByLense()
				...
				if (infoPoints.size() == 2) {
					...
					if(SharedVariables.showTime){
					//System.out.println("SHOW TIME");
						int arcIndex = getArcIndexInLens(1);
						//System.out.println("ARC INDEX: "+arcIndex);
						drawTime(dc,movedScreenCoordinates,arcIndex);
					}
				...
				//colorForUnmodifiedArc = ColorGradient.returnInterpolatedColor(avgTime, minAvgTime, maxAvgTime);
				
				if(SharedVariables.chooseColorConfigurations==1 ){
					colorForUnmodifiedArc = colorAvgTime;
				}else
					if(SharedVariables.chooseColorConfigurations==2 ){
						colorForUnmodifiedArc = colorNumberCalls;
					}else
						if(SharedVariables.chooseColorConfigurations==3 ){
					//		colorForUnmodifiedArc = colorOutlineAvgTime;
						}else
							if(SharedVariables.chooseColorConfigurations==4 ){
								colorForUnmodifiedArc = colorOutlineNumberCalls;
								colorsForModifiedArc = colorPercentNumberCalls;
							}
				
				//colorForUnmodifiedArc = ColorGradient.returnInterpolatedColor(numberCalls, minNumberCalls, maxNumberCalls);
				//colorForUnmodifiedArc =  Color.BLACK;
	...
	private void setCustomColor(int numIntersectingPoints) {
		...
		//Color c = ColorGradient.returnInterpolatedColor(avgTime, minAvgTime, maxAvgTime);
//		Color c = ColorGradient.returnInterpolatedColor(numberCalls, minNumberCalls, maxNumberCalls);
	//	Color[] res = { colorForUnmodifiedArc, Color.BLUE };
	...
	private void drawTime(DrawContext dc, Vec4 screenCoord, int arcIndex) {

	
			//Vec4 screenPoint = SharedVariables.computeScreenCoordinates(p);
			SharedVariables.sr.drawText(dc, screenCoord, time+", #Calls: "+numberCalls,isNumberCallsOutliner,arcIndex);
			sr.draw(dc, screenCoord, 2);
	...
	private int getArcIndexInLens(int numControlPoint){
		return SectorManager.returnIndexArcs(getLineIndex(),numControlPoint);
	}
	...
	public void setMinAvgTime(double minAvgTime) {
		this.minAvgTime = minAvgTime;
	}
	...
	public void setTotNumberCalls(int totNumberCalls) {
		this.totNumberCalls = totNumberCalls;
	}
	
	public void setMinNumberCalls(double minNumberCalls) {
		this.minNumberCalls = minNumberCalls;
	}

	public void setMaxNumberCalls(double maxNumberCalls) {
		this.maxNumberCalls = maxNumberCalls;
	}

	public void setMinTotNumberCalls(double minTotNumberCalls) {
		this.minTotNumberCalls = minTotNumberCalls;
	}

	public void setMaxTotNumberCalls(double maxTotNumberCalls) {
		this.maxTotNumberCalls = maxTotNumberCalls;
	}
	
	public void setNumberCallsOutliner(boolean isNumberCallsOutliner) {
		this.isNumberCallsOutliner = isNumberCallsOutliner;
	}

	public void setAvgTimeOutliner(boolean isAvgTimeOutliner) {
		this.isAvgTimeOutliner = isAvgTimeOutliner;
	}
	
	public void setTotNumberCallsOutliner(int totNumberCallsOutliner) {
		this.totNumberCallsOutliner = totNumberCallsOutliner;
	}

	public void setTotAvgTimeOutliner(int totAvgTimeOutliner) {
		this.totAvgTimeOutliner = totAvgTimeOutliner;
	}
	
	
	public void setMaxNumberCallsPerState(int maxNumberCallsPerState) {
		this.maxNumberCallsPerState = maxNumberCallsPerState;
	}

	public void setMinNumberCallsPerState(int minNumberCallsPerState) {
		this.minNumberCallsPerState = minNumberCallsPerState;
	}

	public void calculateColors(){
		//System.out.println("CALCULATE COLORS");
		colorNumberCalls=ColorGradient.returnInterpolatedColor(numberCalls, minNumberCalls, maxNumberCalls);
		colorAvgTime = ColorGradient.returnInterpolatedColor(avgTime, minAvgTime, maxAvgTime);
		//colorOutlineAvgTime = ColorGradient.returnOutlineColor(isAvgTimeOutliner);
		//colorOutlineNumberCalls = ColorGradient.returnOutlineColor(isNumberCallsOutliner);
		
		//colorOutlineAvgTime = ColorGradient.returnInterpolatedColor(totAvgTimeOutliner, 0, 13);
		colorOutlineNumberCalls = ColorGradient.returnInterpolatedColor(totNumberCallsOutliner, 0, 15);
		//colorPercentNumberCalls = ColorGradient.returnInterpolatedColor((double)numberCalls, 0, (double)totNumberCalls);
		
		//System.out.println("(double)numberCalls, (double)minTotNumberCalls, (double)maxTotNumberCalls"+(double)numberCalls+" - "+ (double)minTotNumberCalls+" - "+ (double)maxTotNumberCalls);
		if(isNumberCallsOutliner){
			colorPercentNumberCalls=Color.RED;
		}else if(numberCalls==0){
			colorPercentNumberCalls=transparent;
		}else{
			colorPercentNumberCalls = ColorGradient.returnInterpolatedColorSingleState((double)numberCalls, (double)minNumberCallsPerState, (double)maxNumberCallsPerState);
