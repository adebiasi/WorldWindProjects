 ...
 */
public abstract class MyAbstractSceneController extends WWObjectImpl implements SceneController
{
	
	static public boolean renderTextureForPointsDetection = false;
	//static String layerEarthName = "WMS Layer";
	//static public String layerEarthName = "Blue Marble May 2004";
	static public String layerEarthName = "Political Boundaries";
...
//    protected Map<String, GroupingFilter> groupingFilters = new HashMap<String, GroupingFilter>();
    ...
    public MyAbstractSceneController()
    {
...
//        dc.applyGroupingFilters();
    ...
    protected void setupShaders(DrawContext dc){
    	 if (MainClass.glsl == null) {
    		 
    		 vertfile = new File("tile_vshader.glsl");
    		 fragfile = new File("my_tile_fshader.glsl");
    		 
         	System.out.println("in preRender: glsl == null");
         	MainClass.glsl = new GLSL(dc.getGL().getGL2());
         	MainClass.glsl.loadVertexShader(vertfile);
         	MainClass.glsl.loadFragmentShader(fragfile);
         }
    }
    */
    ...
    protected void draw(DrawContext dc)
    {
        try
        {
        	...
        	//render the globe
        	//dc.getLayers().getLayerByName("Blue Marble May 2004").setOpacity(0.2);
        	//dc.getLayers().getLayerByName(layerEarthName).setOpacity(1.0);
        	dc.getLayers().getLayerByName(layerEarthName).render(dc); 
         ...
         //   if(SharedVariables.lensIsActive)
        	{
        	renderTextureForPointsDetection=true;
        	if (dc.getLayers() != null){
        		
        		for(String layerName : MyBasicSceneController.layerNames ){
        			System.out.println("layerName: "+layerName);
        		Layer layer = dc.getLayers().getLayerByName(layerName);
        		if(layer.isEnabled()){
        			 layer.render(dc);
        		}
        		}
        	}
        	renderTextureForPointsDetection=false;
            }
        	...
        	// System.out.println("draw layers");
            // Draw the layers.
            if (dc.getLayers() != null)
            {
                for (Layer layer : dc.getLayers())
                {
                    ...
                    try
                    {
                        if (layer != null)
                        {
                        	if(layer.getName().compareTo(layerEarthName)!=0){
           ...
           // System.out.println("drawOrderedSurfaceRenderables ");
            // Draw the deferred/ordered surface renderables.
            this.drawOrderedSurfaceRenderables(dc);

            
        
            //System.out.println("finish drawOrderedSurfaceRenderables");
...
//            dc.applyGroupingFilters();
    ...
    protected void  drawDiagnosticDisplays(){
   ...
   // public static boolean isPreRenderingPhase=false;
    
  
   	public int numAffectedCurves=0;
   	public int numCurrAffectedCurves=0;
    
   	/*
    public void resetAffectedCurves( Iterable<? extends Renderable> renderables){
		 for (Renderable renderable : renderables)
        {
			 ((CubicSplinePolyline)renderable).setAffectedByLense(false);
			 ((CubicSplinePolyline)renderable).setNodeInsideLense(false);
        }
	 }
   	*/
    ...
    /*
    private int countAffectedCurves( Iterable<? extends Renderable> renderables){
		int i=0;
		 for (Renderable renderable : renderables)
         {
			 if(((CubicSplinePolyline)renderable).isAffectedByLense()==true){
			i++;
			 }
         }
